...
def generate_graph(triplets):
    ...
    data.entity = torch.from_numpy(uniq_entity)  # numpy转化成tensor
    data.edge_type = edge_type  # edge_type为无向图
    #data.edge_norm = edge_normalization(edge_type, edge_index_all, len(uniq_entity), num_rels)

    return data

def semeval_sentence_features(model, seme,all_seeds,concept_graphs, relation_map):
    """
        Graph features for each sentence (document) instance in a domain.
    """
    x, dico = sem_get_domain_dataset(seme)
    d = list(dico.values())
    ...
    for j in tqdm(range(len(x)), position=0, leave=False):
        ...
        try:
            ...
            if(len(xg) != 0):
                ...
                if len(xg) > 50000:
                    xg = xg[:50000, :]
                sg = generate_graph(xg).to(torch.device('cuda'))
                features = model(sg.entity, sg.edge_index, sg.edge_type, sg.rel, sg.edge_index_all)
...
def sentence_features(model, domain, split, all_seeds, concept_graphs, relation_map):
    """
        Graph features for each sentence (document) instance in a domain.
    """
...
if __name__ == '__main__':
    ...
    n_bases = 4
    model = RGAT(len(relation_map), num_bases=n_bases, dropout=dropout).cuda()
    ...
    semeval_test = y.tolist()
    '''
    for domain in ['books','electronics', 'dvd', 'kitchen']:
        print ('Extracting features for', domain)
        for split in ['test', 'small']:
            with torch.no_grad():
                sf = sentence_features(model, domain, split, all_seeds, concept_graphs, relation_map)
                np.ndarray.dump(sf, open('graph_features/sf_' + domain + '_' + split + '_5000.np', 'wb'))
                print('Done.')
    '''
    with torch.no_grad():
        sf = semeval_sentence_features(model, semeval_train, all_seeds, concept_graphs, relation_map)
        np.ndarray.dump(sf, open('graph_features/sf_' + 'semeval' + '_' + 'train' + '_5000.np', 'wb'))
