...
def train_model(model, optimizer, loss_class, loss_domain, X_s1, X_s2, Y_s, X_t1, X_t2, al=1):
    ...
    permutation = torch.randperm(2000)
    ...
    for i in range(0, 2000, batch_size):
        ...
        loss_graph = CORAL(x_s2, x_t2)
        loss = loss_class_s + loss_domain_s + loss_domain_t + loss_recon_s + loss_recon_t + loss_graph
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    for d1 in domains:
        ...
        X_t1, Y_t1, X_t2, Y_t2, _ = get_dataset(d1, max_words=bow_size)
        X_s, Y_s = sem_get_dataset(semeval_train)
        Y_s = torch.LongTensor(Y_s)
        Y_t1 = torch.LongTensor(Y_t1)
        Y_t2 = torch.LongTensor(Y_t2)

        # Graph features
        X_s_ = np.load(open('graph_features/sf_semeval_train_5000.np', 'rb'), allow_pickle=True)
        X_t1_ = np.load(open('graph_features/sf_' + d1 + '_small_5000.np', 'rb'), allow_pickle=True)
        X_t2_ = np.load(open('graph_features/sf_' + d1 + '_test_5000.np', 'rb'), allow_pickle=True)

        if transform:

            # c = norm[d1+d2]
            X_s = torch.tensor(np.log(1 + np.array(X_s.todense()).astype('float32')))
            X_t1 = torch.tensor(np.log(1 + np.array(X_t1.todense()).astype('float32')))
            X_t2 = torch.tensor(np.log(1 + np.array(X_t2.todense()).astype('float32')))
        ...
        print(d1, 'semeval', str(max(all_accs)))
